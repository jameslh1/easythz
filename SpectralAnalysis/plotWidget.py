...
darkgrey=(0.184,0.31,0.31)
figBgColour=(0.937,0.922,0.906)
#axisColour=darkgrey
axisColour='w'
axisTextSize=24   # size for text labels by ticks

axisLabelDict={'fontweight':'bold','fontsize':20}   # x and y axis font dictionary
...
#markerStyles=['y-o','y--s','y-.>','y:<','b-o','b--s','b-.>','b:<','r-o','r--s','r-.>','r:<','g-o','g--s','g-.>','g:<']
markerStyles=['y-o','r-s','b->','g-x','y-s','r->','b-x','g-o','y->','r-x','b-o','g-s','y-x','r-o','b-s','g->']
mSt=['y-o','r-s','b->','g-x','y-s','r->','b-x','g-o','y->','r-x','b-o','g-s','y-x','r-o','b-s','g->']
#mSt=['y-o','y--s','y-.>','y:<','b-o','b--s','b-.>','b:<','r-o','r--s','r-.>','r:<','g-o','g--s','g-.>','g:<']
markerEdge  =['k']
...
class Line:
	"""Generic line class"""
	def __init__(self,xdata=[],ydata=[],label='',colour=[],linestyle='-'):
		self.xdata=xdata
		self.ydata=ydata
		self.label=label
		self.colour=colour
...
class PlotWidget:
	"""This is the plotWidget application"""

	ALLOWED_LINESTYLES=['-','--','-.',':','None']

	def __init__(self,win=None,numSubplots=[1,1],figWidth=500,figHeight=500,globalPrm='',makeTB=True,showRect=False,plotPolar=False):
		self.vbox= gtk.VBox(homogeneous=False, spacing=0)
		self.win=win
		self.globalPrm=globalPrm
		self.makeTB = makeTB
...
##################### NOT IN USE ---->
	def createToolbar(self,widget):
		self.myTB=MyToolbar(widget)

		tb=self.myTB.addToToolbar(self.myTB.toolbar,'Generic','Load',gtk.STOCK_OPEN)
		tb=self.myTB.addToToolbar(self.myTB.toolbar,'Generic','Clear',gtk.STOCK_CLEAR)
		tb=self.myTB.addToToolbar(self.myTB.toolbar,'Space')
		self.tbLogY=self.myTB.addToToolbar(self.myTB.toolbar,'Toggle','Log y',gtk.STOCK_INDEX)

	def connectToolbar(self):
		self.myTB.findBtn('Clear').connect("clicked", self.OnClear)
		self.myTB.findBtn('Load').connect("clicked", self.OnLoad)
		self.myTB.findBtn('Log y').connect("clicked", OnLogY, self.axis, self.canvas)
####################### <-- NOT IN USE
...
#			if numCols>2:				
#				plotDlg=plotDialog(self.win,axLabels)
#				[xIndex,yIndex,zIndex]=plotDlg.run()
#				xIndex=[]
#				yIndex=[]
#				for m in range(numCols-1):
#					xIndex.append(0)
#					yIndex.append(m+1)
#				print 'need to create column choice dialog to replace "plotDialog in control.py"'
#			else:
#				xIndex = [0]
#				yIndex = [1]


#			for m in range(len(yIndex)):
			if not xIndex==-1:
				if delim=='xml':
...
#				if not type(ydata[0])==list:
					#if delay mm->ps
#					if axLabels[0]=='Delay (mm)':
#						xdata=xdata/0.15
#						axLabels[0]='Delay (ps)'
...
#				numPlots = len(xIndex)
	...
	def plotContour(self, axis, dataArray, numContours=50, xax=None, yax=None):
		if (xax==None) and (yax==None):
			...
			print 'plotWidget.plotContour():: warning:: specified either x or y but not both'
			return
		else:
...
#		print resultDict[name]
	...
	def newFFTwin(self, widget, mode=0):
		...
		if not fftOptResult:
			print 'PlotWidget.newFFTwin()::getFFToptions dialog was cancelled'
			return
...
#		else:
		...
		if (len(self.fftWinList)==0) or (newWin==1):
			...
			newplot=PlotWidget(numSubplots=self.numFFTsubplots,makeTB=True)
...
#		else:
	...
	def axLabel(self, axis, xlabel, ylabel):
		xlbl=axis.set_xlabel(xlabel,fontdict=axisLabelDict)
		ylbl=axis.set_ylabel(ylabel,fontdict=axisLabelDict)
		self.canvas.draw()
...
#			get value of polynomial
			...
			if result:
				...
				print 'PlotWidget.fitPolynomial()::getFitOptions dialog was cancelled'
				return
...
#		self.btnClear = paramBoxes.objectList[6]
		...
		### preview plot widget
		self.previewPlot = PlotWidget(self.dlg,numSubplots=[1,1,1],figWidth=300,figHeight=300,makeTB=False)
		self.previewPlot.axisList[0].set_position(PREVIEW_PLOT_POSITION)
			...
			#		self.createToolbar(self.vbox)
					...
					for x in range(numRows):
						...
						for y in range(numCols):
							self.plotList.append(PlotWidget(self.win.win,numSubplots=[1,1],makeTB=True,showRect=False))
							if numRows>1:
...
#					print '\n\n*** xlabel=',xAxLabel,', rectangle visible = ', rectVisible
			...
			#		self.createToolbar(self.vbox)
					...
					for x in range(numRows):
						self.hbox = gtk.HBox(homogeneous=False, spacing=0)			
						for y in range(numCols):
							self.plotList.append(PlotWidget(self.win.win,numSubplots=[1,1],makeTB=True,showRect=rectVisible))
							if numRows>1:
								self.plotList[n].vbox.set_size_request(500,300)
							self.hbox.pack_start(self.plotList[n].vbox)
							n=n+1
...
if __name__ == "__main__":
	...
	w = NewWindow(title='Test')
	newplot=PlotWidget()
	w.addWidget(newplot.vbox)
	w.win.connect('destroy',w.OnQuit)
